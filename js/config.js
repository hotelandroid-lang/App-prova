...
async function getClients(filters = {}) {
  let query = supabase.from('clients').select('*').order('cognoms');
...
async function getAnimals(filters = {}) {
  let query = supabase.from('animals').select(`*, clients(nom, cognoms)`).order('nom');
...
async function getAnimal(id) {
  const { data, error } = await supabase.from('animals').select(`*, clients(nom, cognoms, tel_mobil, email)`).eq('id', id).single();
...
async function saveAnimal(animal) {
  ...
  if (animal.id) {
    const { data, error } = await supabase.from('animals').update(animal).eq('id', animal.id).select().single();
...
async function getReserves(filters = {}) {
  let query = supabase.from('reserves')
...
async function getReservesAvui() {
  ...
  const { data: entrades } = await supabase.from('reserves')
  ...
  const { data: sortides } = await supabase.from('reserves')
    .select(`*, clients(nom, cognoms), animals(nom, raca), habitacions(id)`)
...
async function saveReserva(reserva) {
  ...
  if (!reserva.numero) {
    ...
    const { count } = await supabase.from('reserves').select('*', { count: 'exact', head: true });
    reserva.numero = `RES-${any}-${String((count || 0) + 1).padStart(4, '0')}`;
  }
  reserva.updated_at = new Date().toISOString();
  if (reserva.id) {
    const { data, error } = await supabase.from('reserves').update(reserva).eq('id', reserva.id).select().single();
    if (error) console.error('Error update reserva:', error);
    return data;
  } else {
    const { data, error } = await supabase.from('reserves').insert(reserva).select().single();
...
async function getHabitacions() {
  const { data, error } = await supabase.from('habitacions').select('*').order('id');
...
async function getStatsAvui() {
  const avui = new Date().toISOString().split('T')[0];
  const { data: estades } = await supabase.from('reserves')
    .select('id', { count: 'exact' })
    .in('estat', ['confirmada', 'pendent'])
    .lte('data_entrada', avui + 'T23:59:59')
    .gte('data_sortida', avui + 'T00:00:00');
  const { count: totalAnimals } = await supabase.from('animals')
    .select('*', { count: 'exact', head: true });
  const { count: totalHab } = await supabase.from('habitacions')
...
async function getVendes(filters = {}) {
  let query = supabase.from('vendes')
...
async function getTasques() {
  const { data, error } = await supabase.from('tasques')
...
async function toggleTasca(id, completada) {
  const { error } = await supabase.from('tasques').update({ completada }).eq('id', id);
  if (error) console.error('Error toggle tasca:', error);
}

async function saveTasca(tasca) {
  if (tasca.id) {
    const { data, error } = await supabase.from('tasques').update(tasca).eq('id', tasca.id).select().single();
    if (error) console.error('Error update tasca:', error);
    return data;
  } else {
    const { data, error } = await supabase.from('tasques').insert(tasca).select().single();
